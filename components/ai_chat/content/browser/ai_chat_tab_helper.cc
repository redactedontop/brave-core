...
void AIChatTabHelper::OnAllPDFPagesTextReceived(
  ...
  // Store texts in a flat_map to maintain order
  base::flat_map<size_t, std::string> ordered_texts;
  ordered_texts.reserve(page_texts.size());

  // Insert all texts into the map
  ...
  // Iterate in order (flat_map maintains sorted order by key)
  for (const auto& [index, text] : ordered_texts) {
    content.append(text);
    if (index < ordered_texts.size() - 1) {
      content.append("\n");
    }
  }
